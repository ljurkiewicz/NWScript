...
void AddRandFeat(object oPC, object oSum);
...
void GetAdvancedSummon(object oPC)
{
    ...
    if((nClericDomain1 == 1 || nClericDomain2 == 1) && GetLevelByClass(CLASS_TYPE_CLERIC) > 5)  bAdvancedSummon = 1;
    if(GetLevelByClass(CLASS_TYPE_PALE_MASTER) > 5) bAdvancedSummon = 2;
    if(GetHasFeat(FEAT_GREATER_SPELL_FOCUS_CONJURATION, oPC) || ((nClericDomain1 || nClericDomain2 == 1) && (GetLevelByClass(CLASS_TYPE_PALE_MASTER) > 5))) bAdvancedSummon = 3;
  SendMessageToPC(oPC, "GetAdvancedSummon: " + IntToString(bAdvancedSummon));
}
///////////////////////////////////////////////////////
void RandAppearance(object oPC, object oSum)
{
    ...
    if(GetTag(oSum) == "szc_sum001" &&  GetLevelByClass(CLASS_TYPE_DRUID, oPC))
    {
        switch(nSpellID)
        {
            ...
            case SPELL_SUMMON_CREATURE_IX:   nRoll = d10() + 32; break;
         }

       switch (nRoll)
        {   //summon I-IX natural appearances
            ...
            case 11: AppType = 3309; break;//Bear: Great Claw
            case 12: AppType = 1243; break; // Ant:Giant Soldier
            case 13: AppType = 8180; break; // Bear - black
            case 14: AppType = 6472; break; //topiary guard triceratops
            case 15: AppType = 1333; break; // Antelope:Black*
            case 16: AppType = 1299; break; //Cat: Tiger (Storvik)
            case 17: AppType = 1496; break; //Assassin Vine, Vertical
            case 18: AppType = 3300; break; // Anhkheg, medium
            case 19: AppType = 3473; break; // Slug:Large
            case 20: AppType = 1057; break; // Myconid, Elder
            case 21: AppType = 3136; break; // Elemental: Animental: Water Snake
            case 22: AppType = 3136; break; // Elemental: Animental: Air Wolf
            case 23: AppType = 3908; break; //DG_Rhino_Mt
            case 24: AppType = 3135; break; // Animental stone bear
            case 25: AppType = 1178; break; // Scorpion, Giant 2
            ...
            case 39: AppType = 52; break; // Elemental Base:air
            case 40: AppType = 8076; break; // Elemental base:water, elder
            case 42: AppType = 61; break; // Elemental base:fire, elder
            case 43: AppType = 1492; break; // Treant 1
       }
    }
    else if(GetTag(oSum) == "szc_sum001")
    {
     ...
     else if(GetTag(oSum) == "szc_sum002")
     {
        ...
        ///Undead appearances
            ...
            case 16: AppType = 123;  break; // Mohrg 8077 / 123
            ...
            case 23: AppType = 36;   break; // intellect devourer:skeletal 36 / 54
...
void ApplyEffPreset(object oPC, object oSum)
{
    ...
    switch(currentlvl)
    {
        case 1:case 2:case 3:case 4:case 5:nDmg = DAMAGE_BONUS_1d6; break;
        case 6:case 7:case 8:case 9:case 10:nDmg = DAMAGE_BONUS_2d8; break;
        case 11:case 12:case 13:case 14:case 15:nDmg = DAMAGE_BONUS_10; break;
        case 16:case 17:case 18:case 19:case 20: nDmg = DAMAGE_BONUS_2d10; break;
        case 21:case 22:case 23:case 24:case 25: nDmg = DAMAGE_BONUS_2d12; break;
        case 26:case 27:case 28:case 29:case 30: nDmg = DAMAGE_BONUS_20; break;
        //default: nDmg = DAMAGE_BONUS_1d4; break;
    }
   int nAlign = GetAlignmentGoodEvil(oPC);
   ...
   if(nSubtype == 0)  //fire
   {
       ...
       sSubtype = "P³omieni";
       AssignCommand(oSum,ActionCastSpellAtObject(SPELLABILITY_AURA_FIRE, oSum, METAMAGIC_NONE, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, FALSE));
   }
    if(nSubtype == 1)  //cold
   {
       nDamageType = DAMAGE_TYPE_COLD;
       sSubtype = "Mrozu";
       AssignCommand(oSum,ActionCastSpellAtObject(SPELLABILITY_AURA_COLD, oSum, METAMAGIC_NONE, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, FALSE));
   }
    if(nSubtype == 2)  //acid
   {
       nDamageType = DAMAGE_TYPE_ACID;
       sSubtype = "Jadu";
       AssignCommand(oSum,ActionCastSpellAtObject(SPELLABILITY_AURA_MENACE, oSum, METAMAGIC_NONE, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, FALSE));
   }
    if(nSubtype == 3)  //air
   {
       nDamageType = DAMAGE_TYPE_ELECTRICAL;
       sSubtype = "Niebosk³onu";
       AssignCommand(oSum,ActionCastSpellAtObject(SPELLABILITY_AURA_ELECTRICITY, oSum, METAMAGIC_NONE, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, FALSE));
   }
   if(nSubtype == 4)  //dark or light
   {
       int nRoll =  d2();
       if((nAlign == ALIGNMENT_EVIL) || (nAlign == ALIGNMENT_NEUTRAL && nRoll == 1))
       {
           nDamageType = DAMAGE_TYPE_NEGATIVE;
            sSubtype = "Ciemnoœci";
            AssignCommand(oSum,ActionCastSpellAtObject(SPELLABILITY_AURA_BLINDING, oSum, METAMAGIC_NONE, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, FALSE));
        }
        else if((nAlign == ALIGNMENT_GOOD) || (nAlign == ALIGNMENT_NEUTRAL && nRoll == 2))
       {
            nDamageType = DAMAGE_TYPE_POSITIVE;
            sSubtype = "Blasku";
            AssignCommand(oSum, ActionCastSpellAtObject(SPELLABILITY_AURA_PROTECTION, oSum, METAMAGIC_NONE, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, FALSE));
        }
   }

    effect eElem1 = EffectDamageIncrease(nDmg,nDamageType);
    effect eElem2 = EffectDamageImmunityIncrease(nDamageType, 50);
    effect eElem3 = EffectDamageShield((clvl/4), DAMAGE_BONUS_2d6,nDamageType);
...
effect RandEffect(object oPC, object oSum)
{
    ...
    if(nSumType == 1)
    {
        ...
        switch (nRoll)
        {
          ...
          SendMessageToPC(oPC, "nRoll: " + IntToString(nRoll));
    }
    else if(nSumType == 2)
    {
...
void ApplyRandEff(object oPC, object oSum)
{
    int nSpellID = GetLocalInt(oPC, "SPELLID");
    int clvl = GetLocalInt(oPC, "CASTERLVL");
    SendMessageToPC(oPC, "nSpellID " + IntToString(nSpellID));
    GetAdvancedSummon(oPC);
    effect RandEff;

    int iter;
    if (GetTag(oSum) == "szc_sum001")// Summon I_IX
    {
    ...
    if (GetTag(oSum) == "szc_sum002")  //Summon Undead
    {
     ...
     if (GetTag(oSum) == "szc_sum003")  //Summon Outsider
    {
         //ApplyEffPreset(oPC, oSum);
     ...
     if (GetTag(oSum) == "szc_sum004")  //Summon Construct/animate weapon
    {
       if(bAdvancedSummon == 3)
        {
            int nAmount = clvl;
...
void AddRandFeat(object oPC, object oSum)
{
    ...
    {
        nRoll = d8();
        if(GetTag(oSum) == "szc_sum001" || GetTag(oSum) == "szc_sum002" || (GetLocalInt(oSum, "WeaponType") == 0))  //unarmed
        {
            switch (nRoll)
            {
                case 1: nFeat = 62; break; // Impr crit unarm
                case 2: nFeat = 290; break; // weapon spec Creature
                case 3: nFeat = FEAT_EPIC_ARMOR_SKIN; break;
        ...
        else if((GetTag(oSum) == "szc_sum003") && GetLocalInt(oSum, "WeaponType") > 0) //with weapon
         {
           int number = d4();
...
void AddRawAbility(object oPC, object oSum)
{
    ...
    if (GetTag(oSum) == "szc_sum003")
    {
       ...
       NWNX_Creature_SetRawAbilityScore(oSum, ABILITY_DEXTERITY, (nPreDEX + clvl / 2));
       NWNX_Creature_SetRawAbilityScore(oSum, ABILITY_CONSTITUTION, (nPreCON + clvl / 6));
    }
...
void AddLvlProgress(object oPC, object oSum)
{
    ...
    {
        if (bAdvancedSummon == 1 || bAdvancedSummon == 3)
            NWNX_Creature_LevelUp(oSum, CLASS_TYPE_MAGICAL_BEAST, currentlvl - 1);
        else
    ...
    {
        if (bAdvancedSummon == 2 || bAdvancedSummon == 3)
            NWNX_Creature_LevelUp(oSum, CLASS_TYPE_MONSTROUS, currentlvl - 1);
        else
...
void CreateWeaponForSummon(object oPC)
{
    ...
    SendMessageToPC(oPC, "nWeapon: " + IntToString(nBaseItem));
    SendMessageToPC(oPC, "sSubtype: " + sSubtype);
    ...
    if(sSubtype == "P³omieni") nVisual = ITEM_VISUAL_FIRE;
    if(sSubtype == "Niebosk³onu") nVisual = ITEM_VISUAL_ELECTRICAL;
    if(sSubtype == "Jadu") nVisual = ITEM_VISUAL_ACID;
    if(sSubtype == "Ciemnoœci") nVisual = ITEM_VISUAL_EVIL;
    if(sSubtype == "Blasku") nVisual = ITEM_VISUAL_HOLY;
    if(sSubtype == "Mrozu") nVisual = ITEM_VISUAL_COLD;
